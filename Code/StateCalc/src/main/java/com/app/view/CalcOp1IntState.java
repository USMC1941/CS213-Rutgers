...
/**
 * This class represents the integer first operand state. It is a subclass of CalcState,
 * implementing the State design pattern for the Calculator. This class also implements the
 * Singleton design pattern.
 *
 * <p>As in the clear state, all buttons except '=' are enabled.
 *
...
class CalcOp1IntState extends CalcState {
    ...
    /**
     * Called when the state is entered. Updates display with single digit. Digit buttons are always
     * enabled. Decimal, 'C', and operator buttons must already be enabled at the time this state is
     * entered, and they remain enabled. The '=' must already be disabled, and remains disabled.
     */
    void enter() {
    ...
    /**
     * Processes an event that is fired when in digit state. Event source is a digit, decimal, an
     * operator, or clear. If source is clear, transitions to clear state. If source is decimal,
     * transitions to op1 float state. If source is digit, appends digit to display and stays in
     * this state. If source is an operator, gets and stores operand1, and transitions to operator
     * state.
     *
     * @return New state
     */
