...
/**
 * This class acts as listener for all events fired from the calculator buttons. It keeps track of
 * the current state instance of the calculator model. When an event is handled by the controller,
 * it delegates event processing to the current state instance, which processes the event and
 * returns the next state. When the calculator starts up, it is set to the clear state.
 *
...
public class CalcController {
    ...
    @FXML protected TextField display;

    @FXML protected DigitController digitsController;

    @FXML protected OperatorController opsController;

    @FXML protected ResultController resController;
