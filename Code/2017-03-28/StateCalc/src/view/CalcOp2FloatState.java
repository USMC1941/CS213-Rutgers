...
/**
 * This class represents the floating point first operand state. 
 * It is a subclass of CalcState, implementing the State design pattern for
 * Calculator. This class also implements the Singleton design pattern.
 * <p>
 * In this state all buttons are enabled, except '.'
 *
...
class CalcOp2FloatState extends CalcState {
   ...
   /**
    * Called when the state is entered. Updates display by appending '.' to existing text.
    * Operator and '=' buttons are enabled. The decimal button is disabled.
    */
   void enter() {
      // append '.' to display
    ...
    * Event source is a digit, or clear, or the '=' button.
    * If source is clear, transitions to clear state.
    * If source is digit, appends digit to display and stays in this state.
    * If source is '=', transitions to compute state after storing second operand
    * If source is operator, compute result, set op1 as result, and go to operator state
    *
    * @return New state
    */
