...
/**
 * This class represents the operator state.
 * It is a subclass of CalcState, implementing the State design pattern for
 * Calculator.
 * <p>
 * In this state all operators are disabled. The digits continue to be enabled, and so
 * does the clear button. The '= button continues to be disabled.
 * The decimal button is enabled. 
 * Display is unchanged from before.
 * Operator is stored internally.
 *
...
class CalcOperatorState extends CalcState {
    ...
    * Digit buttons and 'C' button must already be enabled at the time this state is
    * entered, and remain enabled. The '=' button remains disabled.
    * The decimal button is enabled. The operator buttons are disabled.
    */
   void enter() {
   ...
   /**
    * Processes an event that is fired when in this state.
    * Event source is a digit, a decimal or clear.
    * If source is clear, transitions to clear state.
    * If source is digit, transitions to op2 int state.
    * If source is decimal, transitions to op2 float state.
    *
    * @return New state
    */
