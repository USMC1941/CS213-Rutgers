...
/**
 * This class represents the equals state. In this state, the display shows op1, and all buttons
 * except = are enabled. It is a subclass of CalcState, implementing the State design pattern for
 * Calculator.
 *
...
class CalcEqualsState extends CalcState {
    ...
    /**
     * Called when the state is entered. Sets display to op1. Digit, operator, and C buttons
     * continued to be enabled. The decimal button is enabled, and the '=' button is disabled.
    ...
    /**
     * Processes an event that is fired when in equals state. Event source is anything but the '='
     * button. If source is clear, then go to clear state. If source is decimal, then go to op1
     * float state If source is digit, then go to op1 int state If source is operator, then go to
     * operator state
     *
     * @return New state
     */
